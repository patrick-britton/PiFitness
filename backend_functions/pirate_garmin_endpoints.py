"""
Inlined pirate-garmin endpoint registry.

Extracted from `pirate-garmin_clone/src/pirate_garmin/endpoints.py` so the
backend no longer depends on the cloned package being importable at runtime.

The original module imported `ProfileBundle` (from `auth`) and `Host` (from
`client`). Both symbols are provided by the inlined auth module:
`backend_functions.pirate_garmin_auth`.

Usage:
    from backend_functions.pirate_garmin_endpoints import (
        Endpoint,
        ENDPOINTS,
        resolve_endpoint,
        render_endpoint,
...
from backend_functions.pirate_garmin_auth import Host, ProfileBundle
