 ...
 * DBSessions Component
 */
export default function DBSessions() {
  ...
  if (error) {
    return (
      <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md p-4">
        <p className="text-red-700 dark:text-red-300">Failed to load DB sessions: {String(error)}</p>
      </div>
    );
  }
    ...
    <div>
      ...
      {sessions.length === 0 ? (
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md p-6 text-center">
          <p className="text-green-700 dark:text-green-300 font-medium">No active sessions</p>
          <p className="text-sm text-green-600 dark:text-green-400 mt-1">All queries appear idle or complete.</p>
        </div>
      ) : (
