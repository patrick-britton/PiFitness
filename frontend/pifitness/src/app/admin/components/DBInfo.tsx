...
export default function DBInfo() {
  ...
  return (
    <div>
      {/* Sub-tab Navigation — Desktop (full labels, horizontal) */}
      {isDesktop && (
      ...
      {/* Sub-tab Navigation — Portrait (accordion dropdown) */}
      {isPortrait && (
        <div className="mb-4">
          <button
      ...
      {/* Sub-tab Navigation — Landscape (icon-only horizontal scroll) */}
      {isLandscape && (
        <div className="border-b border-gray-200 dark:border-gray-700 mb-4">
          <nav className="flex overflow-x-auto" aria-label="DB Info Sub-tabs">
            {SUB_TABS.map((tab) => (
